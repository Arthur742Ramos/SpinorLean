...
$\Spin(2) \simeq U(1)$ and $\Spin(3) \simeq SU(2)$, together with an
injective homomorphism $\Spin(4) \to SU(2) \times SU(2)$ built from the
...
\begin{itemize}
  ...
  \item The next compact step, an injective homomorphism
        $\Spin(4) \to SU(2) \times SU(2)$, is packaged as
        \code{spinGroupRealCl04ToUnitaryQuaternionPair} on top of the algebra
        equivalence \code{realEvenCl04EquivQuaternionProd : Cl^+(0,4) \simeq
        \mathbb{H} \times \mathbb{H}}; the diagonal compact subgroup is now
        constructed explicitly through
        \code{unitaryQuaternionToSpinGroupRealCl04Diagonal}, and the
        anti-diagonal complex circle through
        \code{unitaryComplexToSpinGroupRealCl04Antidiagonal}.
\end{itemize}
...
\code{realCl03EquivQuaternionProd} and
\code{realEvenCl04EquivQuaternionProd}. The next group-level scaffold is
also now present as
\code{spinGroupRealCl04ToUnitaryQuaternionPair}, an injective homomorphism
from $\Spin(4)$ into pairs of unit quaternions. The remaining crisp
milestone is to prove the missing surjectivity statement and package the
full equivalence. Only after that does it make sense to pursue
$\Spin(5)$ and $\Spin(6)$.
