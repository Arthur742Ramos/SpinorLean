...
square scalings and prove their weight-$2$ torus law. Using those lifts, we
show that any split Levi lift acts projectively as the natural exterior action,
prove an exact split-line image theorem identifying the image with the
square-scaling subgroup, and in higher rank prove a square-determinant Levi
factorization in the orthogonal action. The artifact comprises $20$ Lean source
files totaling $14{,}093$ lines with no \texttt{sorry}.
\end{abstract}
...
square-determinant Levi factorization become machine-checked results rather than
informal consequences of the abstract theory.
...
hyperbolic/Witt data to theorem-level covering statements. The two centerpiece
mathematical outputs are the structural Levi-action theorem and the exact
split-line image theorem. Supporting these are the positive split-rank
kernel/projective-descent package, the explicit hyperbolic transvection and
chosen-line square-scaling lifts with their weight-$2$ torus laws, and the
higher-rank square-determinant Levi factorization in the orthogonal action. The
classical chosen-model equivalence and half-spin package is included as
...
results proved below should be read as the structural extension of that
field-sensitive rank-$1$ picture.
...
and the higher-rank square-determinant factorization proved here.
...
semidirect structure; the theorem that split Levi lifts act projectively as the
natural exterior action; the higher-rank square-determinant Levi consequence;
and the exact split-line image theorem. The point is not abstract generation by
itself, but a concrete chosen-model realization of a root subgroup, its square
torus, and the resulting square-class obstruction.
...
bridge and the exact split-line image theorem; the supporting lift and
descent statements are listed afterward.
\begin{enumerate}
...
\item the higher-rank square-determinant Levi factorization of
...
\item the explicit hyperbolic transvection lift
  ...
  Clifford-level torus conjugation law
...
Lean LOC under \code{Spinor/} & $14{,}093$ \\
...
\begin{theorem}[Square-determinant Levi factorization in the orthogonal action]
\label{thm:square-det-levi}
Assume $\dim W\ge 2$, and let
\[
  \Lambda:\operatorname{GL}(W)\hookrightarrow \SO(\dualProd_K W)
\]
denote the transported Levi embedding $\Lambda(g)=(g^{-{\vee}},g)$. If
$g\in \operatorname{GL}(W)$ has $\det(g)\in (K^\times)^2$, then after choosing a
basis of $W$ and a distinguished basis line, $\Lambda(g)$ factors in
$\SO(\dualProd_K W)$ as
\[
  \begin{aligned}
  \Lambda(g) ={}&
...
representatives implementing this factorization on $W^*\oplus W$. In
particular, $\Lambda(\operatorname{SL}(W))$ is contained in the subgroup of
...
factorization theorem from Section~\ref{sec:results}.
...
the claimed square-determinant Levi factorization.
\end{proof}
...
lifts feed into that bridge, and the higher-rank square-determinant
factorization together with the exact split-line image theorem are its main
consequences. Read that way, the paper is less about rederiving $\bigwedge W$
and more about identifying how much of the split Levi picture is already
visible inside $\Spin$ over a general field. The next mathematical frontier is
to replace the present higher-rank factorization theorem by an exact
higher-rank image theorem, ideally phrased in terms of the subgroup generated by
the explicit root/torus elements or, more conceptually, by the corresponding
spinor-norm condition. On the formalization side, that next step would also
require strengthening the current Clifford/orthogonal-action infrastructure in
\Mathlib so that the image theorem can be stated and reused as cleanly as the
split-line square-scaling result.
