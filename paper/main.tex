...
The headline formalized results are these. In positive split rank, the kernel
of $\Spin(V,Q)\to\SO(V,Q)$ is $\{\pm1\}$, so the exterior-model spin action is
projective but not linear on the orthogonal image. In the hyperbolic model we
construct explicit Clifford lifts of hyperbolic transvections and chosen-line
square scalings, prove their weight-$2$ torus law, and normalize the resulting
chosen-model exterior action of the torus lift. Using those lifts, we show that
any split Levi lift acts projectively as the natural exterior action, prove an
exact split-line image theorem identifying the image with the square-scaling
subgroup, and in higher rank show that every square-determinant Levi element
admits both an orthogonal factorization into transported transvections plus one
chosen-line square scaling and an explicit even unitary Clifford unit acting on
the chosen model by a scalar-normalized exterior action. The artifact comprises
$22$ Lean source files totaling $15{,}035$ lines with no \texttt{sorry}.
\end{abstract}
...
The paper develops the classical construction of a spinor
module $S=\bigwedge W$ attached to an explicit hyperbolic presentation
$Q \simeq \dualProd_K W$, transports it through Witt-style reformulations, and
obtains the induced $\Spin(V,Q)$-action. A crucial point throughout is that the
paper's spinor module is \emph{not} the ambient exterior algebra on $V$, but
...
From the formalization point of view, the paper's claim is that this
chosen-model, field-sensitive covering package can be carried out in
\Lean/\Mathlib all the way to explicit theorem-level image statements. The main
...
From a formalization-venue perspective, the contribution is a reusable
chosen-model interface that transports Clifford and spin actions from explicit
hyperbolic/Witt data to theorem-level covering statements. The three
centerpiece mathematical outputs are the structural Levi-action theorem, the
exact split-line image theorem, and the higher-rank square-determinant
factorization together with its explicit chosen-model lift. Supporting these
are the positive split-rank kernel/projective-descent package, the explicit
hyperbolic transvection and chosen-line square-scaling lifts with their
weight-$2$ torus laws. The classical chosen-model equivalence and half-spin
package is included as
infrastructure on which these field-sensitive theorems rest. The detailed
theorem list appears below in ``New results at a glance''.
...
For readers interested mainly in the venue-facing content, the nonclassical
theorem package of this paper is as follows. Logically, the story runs from
explicit root/torus lifts to the structural Levi-action theorem, and from there
to the higher-rank and split-line image consequences.

From the paper-facing point of view, however, the headline outputs are the Levi
bridge, the exact split-line image theorem, and the constructive higher-rank
square-determinant theorem; the supporting lift and descent statements are
listed afterward.
\begin{enumerate}
...
Repository & blinded for review; public URL restored in the final version \\
...
At the time of writing, the full repository rebuilds with \code{lake build}.
...
For venue purposes, the reusable payoff can be summarized in three layers.
...
action. The decisive point is that the paper's ``spinor module'' is not the
...
The paper's main theorems are mirrored directly by reusable Lean declarations.
...
\end{quote}
These declarations are representative of the paper's design goal: theorem-level
...
The paper deliberately separates the presentation-dependent and split-rank
...
\ref{thm:square-det-levi} capture the paper's main theorem-level arc: a
...
The previous theorem is structural: it identifies the chosen-model action of
...
$2$~\cite[Ch.~IX, \S8, Thm.~8.1]{HoffmanKunze1971} then writes $g$ as basis
...
\]
So the induced orthogonal action rescales the two isotropic lines by
...
The paper uses the classical exterior-model construction only as the foundation
...
$\det(g)=u^2$. Read that way, the paper is less about rederiving $\bigwedge W$
and more about identifying how much of the split Levi picture is already
visible inside explicit Clifford formulas over a general field. The next
