...
This gives a faithful Clifford action immediately. The refined construction via a maximal
totally isotropic subspace is intentionally exposed through the presentation layer rather than by
changing this ambient alias: see `Spinor.Presentation` for the packaged chosen-model API attached
either to an explicit hyperbolic presentation `Q ≃ dualProd K W`, to split data consisting of a
half-dimensional totally isotropic subspace plus a chosen complement via
`HyperbolicPresentation.ofIsCompl`, or, more specifically, to an explicit Witt presentation
`Q ≃ dualProd K Q.wittSubspace`; in split rank, the canonical Witt model can also choose its
complement internally via `splitWittPresentation`. At the lower linear-algebra level,
`Spinor.WittDecomp` provides the general linear Witt decomposition
