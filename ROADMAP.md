...
- [x] Package the ambient `SO(V,Q)`-valued spin map, its kernel, and the conditional covering theorem
  ...
  - [x] package the split-Levi determinant square-class character as an actual quotient
    ...
    square-class products, repeated-pair square-class cancellation, and duplicated-product
    square-class triviality; a full orthogonal-group spinor-norm API remains outside
  ...
  - image-classification statements claimed by this artifact are exact on the split line and
    ...
    order-invariant, repeated-pair-stable, duplicated-product-stable Clifford vector-product norm
    substrate are intentionally outside the submitted theorem surface
