...
- [x] Package the ambient `SO(V,Q)`-valued spin map, its kernel, and the conditional covering theorem
  ...
  - image-classification statements claimed by this artifact are exact on the split line and
    ...
    factorization-existence Clifford/Lipschitz vector-product norm substrate, including separated
...
- submission-ready paper package now lives at `paper/` (`main.tex`, `refs.bib`, `README.md`) with
  ...
  package now reaches factorization-existence Lipschitz-group products when `2` is invertible,
  while recursive
